...
/**
 * Interpretation provider for Capella binary expression element.
 */
public class BinaryExpressionInterpretationProvider extends BaseDataModelElementInterpetationProvider
	...
	public DataModelElement interpretModelElement(final DataModel dataModel, final DataPackage currentDataPackage,
		...
		final BinaryExpression element = (BinaryExpression) interpretedElement;
		...
		if (resolvedValue == null) {
			issues.add(new Issue(Issue.Kind.Error,
					"Could not resolve the value of binary expression " + interpretedElement.getName(),
					interpretedElement));
			return null;
		}
		...
		if (valueType == null) {
			issues.add(new Issue(Issue.Kind.Error,
					"Could not resolve the type of binary expression " + interpretedElement.getName(),
					interpretedElement));
			return null;
		}
