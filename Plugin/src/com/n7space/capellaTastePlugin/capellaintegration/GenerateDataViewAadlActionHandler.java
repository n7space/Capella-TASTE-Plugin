...
/**
 *
 * Handler for "Generate DataView AADL" action registered in plugin.xml.
 *
 * Captures execution events for which it is registered in plugin.xml and
 * invokes the "Generate DataView AADL" action.
 *
 */
public class GenerateDataViewAadlActionHandler extends AbstractHandler {

	/**
	 * Captures an execution event, gathers the required information from the
	 * selection and PathVault and invokes the "Generate DataView AADL" action. Upon
	...
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		...
		if (architecture == null) {
			presenter.presentMessage(MessageKind.Information, "Please select the architecture",
					"Please select the architecture to generate the DataView for. Please note that the architecture must be already exported");
			return null;
		}
